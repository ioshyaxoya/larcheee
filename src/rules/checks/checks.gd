...
## extras: [{label_key, value}] — видимые модификаторы извне (ритм номера и т.п.).
## advantage: два кубика, берётся лучший; оба показываются.
func roll(character: Character, spec: Dictionary, extras: Array = [], advantage: bool = false) -> CheckResult:
	...
	if advantage:
		r.advantage = true
		r.die_second = rng.d20()
		if r.die_second > r.die:
			var swap := r.die
			r.die = r.die_second
			r.die_second = swap
	...
	for x in extras:
		r.modifiers.append({"label_key": str(x.get("label_key", "")), "param": "", "name_key": "", "value": int(x.get("value", 0))})
